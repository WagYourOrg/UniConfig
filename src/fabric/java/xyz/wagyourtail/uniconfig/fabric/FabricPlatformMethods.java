...
public class FabricPlatformMethods implements PlatformMethods {
    private static final Logger LOGGER = LoggerFactory.getLogger(FabricPlatformMethods.class);
    ...
    public void sendFeedback(SharedSuggestionProvider suggestionProvider, Component component, boolean broadcast) {
        ...
        } else if (suggestionProvider instanceof FabricClientCommandSource) {
            ((FabricClientCommandSource) suggestionProvider).sendFeedback(component);
        } else {
    ...
    public void sendFailure(SharedSuggestionProvider suggestionProvider, Component component) {
        if (suggestionProvider instanceof CommandSourceStack) {
            ...
            ((FabricClientCommandSource) suggestionProvider).sendError(component);
        } else {
            LOGGER.warn("Unknown command builder type: {}", suggestionProvider.getClass().getName());
        }
    }
